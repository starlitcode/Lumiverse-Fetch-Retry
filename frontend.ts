...
export function setup(ctx: Ctx, opts?: any) {
  ...
  const isRateLimit = (err: any) =>
    !!err && /\b429\b|rate.?limit|too many requests|quota|overloaded/i.test(String(err));
  ...
  const find = (selector: string): any => {
    let el = null;
    try { el = ctx && ctx.dom && ctx.dom.query ? ctx.dom.query(selector) : null; } catch (_) {}
    if (!el && typeof document !== 'undefined') { try { el = document.querySelector(selector); } catch (_) {} }
    return el;
  };
  ...
  function scheduleRetry(chatId: string, reason: string, err?: any) {
    ...
    s.timer = setTimeout(() => {
      ...
      if (!fireRetry()) { s.selfTriggered = false; s.attempts = 0; }   // click failed, do not leave stale state
    }, delay);
  }
  ...
  try {
    if (ctx?.ui?.registerInputBarAction) {
      ...
      disposers.push(() => { try { action.destroy(); } catch (_) {} });
    } else {
  ...
  return () => {
    ...
    disposers.forEach((d) => { try { d && d(); } catch (_) {} });
    if (modalHandle) { try { modalHandle.dismiss(); } catch (_) {} modalHandle = null; }
    ...
    try {
      if (typeof document !== 'undefined' && document.getElementById) {
        ...
        if (t) { clearTimeout(t.__h); if (t.remove) t.remove(); }
      }
    } catch (_) {}
  };
}
